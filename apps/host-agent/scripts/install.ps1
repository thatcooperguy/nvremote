...
<#
.SYNOPSIS
    Installs the GridStreamer Host Agent as a Windows service.
...
.PARAMETER BinaryPath
    Path to the built gridstreamer-agent.exe binary. Defaults to the binary in the current directory.

.PARAMETER ControlPlaneURL
    URL of the GridStreamer control plane. If not provided, the script will prompt.
...
param(
    [string]$BinaryPath = ".\gridstreamer-agent.exe",
...
$InstallDir = "C:\ProgramData\GridStreamer"
$AgentBinary = Join-Path $InstallDir "gridstreamer-agent.exe"
...
if ([string]::IsNullOrEmpty($ControlPlaneURL)) {
    ...
    $ControlPlaneURL = Read-Host "Enter the Control Plane URL (e.g., https://gridstreamer.example.com)"
}
...
$configContent = @"
# GridStreamer Host Agent Configuration
# Generated by install.ps1 on $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")

control_plane_url: "$ControlPlaneURL"
bootstrap_token: "$BootstrapToken"
host_name: "$hostname"
streamer_path: "C:\\Program Files\\GridStreamer\\gridstreamer-host.exe"
stun_servers:
