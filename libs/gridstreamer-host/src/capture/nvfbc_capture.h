...
//
// Uses NVIDIA's proprietary NvFBC API to capture the desktop directly from
// the GPU framebuffer.  This is the lowest-latency capture method on NVIDIA
...
//
// NvFBC is an NVIDIA-internal API.  We declare just enough structs and
// function signatures here so we can load the DLL at runtime without
// requiring the SDK headers at compile time.  These definitions are
// compatible with NvFBC SDK 7.x / 8.x.
// ---------------------------------------------------------------------------
