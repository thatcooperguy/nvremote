...
**Description:** An attacker gains streaming access to an nvstreamer host without
...
**Mitigations:**
- JWTs are signed with RS256 (RSA 2048-bit keys). Signing key is stored in AWS
  Secrets Manager and rotated every 90 days.
...
- The nvstreamer.exe process binds exclusively to the WireGuard tunnel interface IP,
...
- The gateway relays WireGuard traffic that is encrypted end-to-end between the client
  ...
  specific keys.
- Gateway instances are deployed across multiple availability zones with NLB
  (Network Load Balancer) distribution. DDoS is mitigated by AWS Shield Standard
  (included) and optionally AWS Shield Advanced.
...
- Gateway instances are hardened: minimal OS (Amazon Linux 2023), no SSH access in
  production, immutable infrastructure (replaced, never patched in place).
...
availability (DDoS resilience depends on AWS Shield tier).
...
- Compromising AWS credentials to access the infrastructure
...
- Control plane runs on ECS Fargate with no SSH access. Container images are signed
  and scanned for vulnerabilities in CI/CD.
- Database credentials are stored in AWS Secrets Manager and rotated automatically.
  The application retrieves them at startup via IAM role-based access.
...
- Rate limiting is enforced at the ALB and application layers to prevent brute-force
  attacks.
- Web Application Firewall (WAF) rules block common attack patterns (SQLi, XSS, path
  traversal).
- Security group rules limit inbound traffic to port 443 from the ALB only.
...
```
Inbound:  ALLOW TCP 443 from ALB security group only
Inbound:  DENY ALL other
Outbound: ALLOW TCP 5432 to RDS security group (PostgreSQL)
Outbound: ALLOW TCP 6379 to ElastiCache security group (Redis)
...
| **Sensitive** | Refresh tokens, mTLS certificates | Encrypted OS keychain / AWS Secrets Manager | AES-256 at rest |
| **Internal** | User profiles, org data, session records | PostgreSQL (RDS) | AES-256 at rest (RDS encryption) |
| **Audit** | Audit log entries | PostgreSQL + S3 archives | AES-256 at rest, immutable |

### Encryption at Rest

| Store | Encryption | Key Management |
|---|---|---|
| RDS PostgreSQL | AES-256 (AWS-managed KMS key) | Automatic rotation |
| S3 (audit archives) | AES-256 (SSE-S3) | AWS-managed |
| ElastiCache Redis | AES-256 (at-rest encryption enabled) | AWS-managed |
...
| **Retention** | 90 days in PostgreSQL (hot storage, queryable via API). Archived to S3 daily in JSONL format (cold storage, retained for 7 years). |
...
| **HIPAA** | Potential use in healthcare settings | BAA with AWS in place. Application-level controls (audit logging, encryption, access controls) meet technical safeguard requirements. |
