...
NVIDIA Remote Stream enables secure remote access to `nvstreamer.exe` hosts over the
...
running NVIDIA's nvstreamer software, regardless of network topology or NAT
configuration.
...
```
User        Client App       Control Plane       Gateway        Host Agent    nvstreamer
 |              |                  |                 |               |              |
 |--Sign In---->|                  |                 |               |              |
 |              |--OIDC Auth------>|                 |               |              |
 |              |<--JWT------------|                 |               |              |
 ...
 |              |                  |                 |               |              |
 |              |--nvstreamer traffic (encrypted)----|-------------->|--local IPC-->|
 |              |                  |                 |               |              |
```
...
- **Key Responsibilities:**
  ...
  - Launch nvstreamer client pointed at tunnel IP
...
- **Purpose:** Runs on each nvstreamer host machine. Manages the host's participation
  in the overlay network and interfaces with the local nvstreamer.exe process.
- **Key Responsibilities:**
  ...
  - Monitor nvstreamer.exe process health
  - Tear down tunnel on session termination
- **Local Dependencies:** WireGuard tools, nvstreamer.exe

### 5. nvstreamer.exe

- **Runtime:** NVIDIA proprietary binary
- **Purpose:** The actual GPU streaming engine. Captures GPU frames, encodes via
  NVENC, and streams to a connected client.
- **Integration:** The Host Agent does not modify or wrap nvstreamer.exe. It ensures
  nvstreamer is running and listening on the WireGuard tunnel interface IP. The client
  connects to nvstreamer over the tunnel as if it were on a local network.
...
----  -----            ------
 ...
 9    Host Agent       Ensures nvstreamer.exe is listening and bound to 10.100.42.3.
...
15    Client App       Launches nvstreamer client, connecting to 10.100.42.3.
                       Streaming begins.
...
| **Host Agent** | Go | 1.22+ | Compiles to a single static binary with no runtime dependencies -- critical for deployment on customer Windows machines. Low memory footprint. Excellent concurrency primitives for managing WireGuard and nvstreamer subprocesses. |
...
| **Infrastructure** | Terraform + AWS | Terraform 1.7+, AWS | Declarative infrastructure as code. AWS provides global presence for gateway nodes (low latency from any region). ECS Fargate for control plane (serverless containers). |
...
| `hosts` | Registered nvstreamer host machines. Each belongs to one org. |
...
- **Data Path** (overlay network): WireGuard UDP tunnels. Carries nvstreamer video/
...
|                                                       |
|  +------------------+                                 |
|  |  EC2 / NLB       |    Gateway instances            |
|  |  (UDP 51820)     |    (one per AZ, auto-scaled)    |
|  +------------------+                                 |
|                                                       |
|  +------------------+                                 |
|  |  ElastiCache     |    Session state cache,         |
|  |  (Redis)         |    rate limiting                |
|  +------------------+                                 |
|                                                       |
|  +------------------+                                 |
...
|---|---|---|
| Control Plane | Horizontal (ECS task count) | CPU > 60% or request latency > 200ms |
| Gateway | Horizontal (EC2 ASG) | Active tunnel count > 500 per instance |
| Database | Vertical (RDS instance class) + read replicas | Connection count or query latency |
| Redis | Vertical (ElastiCache node type) | Memory usage > 75% |
