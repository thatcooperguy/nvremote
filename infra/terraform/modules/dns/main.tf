##############################################################################
# CrazyStream — DNS & Domain Mapping Module
#
...
# ---------------------------------------------------------------------------
resource "google_dns_managed_zone" "crazystream" {
  name        = "${var.project_name}-dns-zone"
  dns_name    = "${var.domain_name}."
  project     = var.project_id
  description = "CrazyStream public DNS zone for ${var.domain_name}"
  visibility  = "public"
...
resource "google_dns_record_set" "apex_cname" {
  name         = google_dns_managed_zone.crazystream.dns_name
  type         = "A"
  ttl          = 300
  managed_zone = google_dns_managed_zone.crazystream.name
  project      = var.project_id
...
resource "google_dns_record_set" "api_cname" {
  name         = "api.${google_dns_managed_zone.crazystream.dns_name}"
  type         = "CNAME"
  ttl          = 300
  managed_zone = google_dns_managed_zone.crazystream.name
  project      = var.project_id
...
resource "google_dns_record_set" "www_cname" {
  name         = "www.${google_dns_managed_zone.crazystream.dns_name}"
  type         = "CNAME"
  ttl          = 300
  managed_zone = google_dns_managed_zone.crazystream.name
  project      = var.project_id
